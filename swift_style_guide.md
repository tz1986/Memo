...
Where an Xcode project is involved, the organization should be set to `sudofluff` and the Bundle Identifier set to `com.sudofluff.AppName` where `AppName` is the name of the application project.

## SKU

Use the same naming convention for Bundle ID: `com.sudofluff.AppName` where `AppName` is the name of the application project.

## Versions Control

Use agvtool to update or increment version number of the app. The naming convention is `major.minor.patch`. Xcode will convert the app's version by using 'version number' and 'build number' as `version_number.build_number`.
